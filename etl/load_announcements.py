...
-------------------------
Spiral 2 — Load announcements (trd_buy) by organizer BIN.
"""
...
logger = logging.getLogger("etl.announcements")
...
def in_range(dt, year_from: int, year_to: int) -> bool:
    if not dt:
        return False
        ...
        async def process_bin(bin_code: str) -> None:
            async with sem:
                async with pool.acquire() as conn:
                    ...
                    try:
                        state_last_id, state_resume_path = await get_etl_state(conn, "announcements", bin_code)
                        backstep = args.resume_backstep if args.resume_backstep is not None else cfg.resume_backstep
                        base_path = f"/v3/trd-buy/bin/{bin_code}"
                        ...
                        async for items, next_page in client.iter_rest_pages(base_path, start_path=start_path):
                            ...
                            for ann in items:
                                ...
                                ann_id = safe_int(ann.get("id"))
                                if ann_id is not None:
                                    page_last_id = ann_id

                                publish_dt = parse_dt(ann.get("publish_date"))
                                if not in_range(publish_dt, year_from, year_to):
                                    skipped += 1
                                    continue
                            ...
                            if not args.dry_run:
                                ...
                                await update_etl_state(conn, "announcements", bin_code, new_last_id, resume_path)
                        ...
                        errors += 1
                        logger.exception("BIN %s: fatal announcements ingestion error: %s", bin_code, e)
                    ...
                    logger.info("BIN %s: announcements upserted=%d fetched=%d skipped=%d errors=%d", bin_code, upserted, fetched, skipped, errors)
...
if __name__ == "__main__":
    asyncio.run(load_announcements())
